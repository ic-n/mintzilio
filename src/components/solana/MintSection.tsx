import { usePhantom, useModal, useSolana, useDisconnect } from '@phantom/react-sdk';
  ...
  return (
    <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6 py-20">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(160deg, rgba(12,12,26,0.92), rgba(18,18,42,0.88))',
          border: '1px solid rgba(192,192,216,0.12)',
          boxShadow: '0 32px 80px rgba(0,0,0,0.7), 0 0 0 1px rgba(192,192,216,0.06)',
          backdropFilter: 'blur(24px)',
        }}
      >
        {/* Badge */}
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-cyan-neon" style={{ boxShadow: '0 0 8px #00FFEE' }} />
        ...
        <div>
          ...
          >
            <span style={{
              background: 'linear-gradient(90deg, #C0C0D8 0%, #ffffff 40%, #C0C0D8 80%)',
        ...
        <div
          ...
          style={{ background: 'rgba(192,192,216,0.04)', border: '1px solid rgba(192,192,216,0.08)' }}
        >
          <div className="flex justify-between items-baseline mb-3">
            <span className="text-chrome-dim text-xs font-display font-bold tracking-widest uppercase">Supply</span>
            {cmLoading
              ? <span className="text-chrome-dim/40 text-sm">Loading…</span>
              : state
                ? <span className="font-display font-bold text-chrome">
                    {state.redeemed} <span className="text-chrome-dim/50 font-normal">/ {state.available}</span>
                  </span>
                : <span className="text-chrome-dim/40 text-sm">—</span>
            }
          </div>
          {!cmLoading && state && (
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(192,192,216,0.1)' }}>
              <div
                className="h-full rounded-full transition-all duration-700"
                style={{
                  width: `${(state.redeemed / state.available) * 100}%`,
                  background: isSoldOut ? 'linear-gradient(90deg,#FF00BB,#ff5f1f)' : 'linear-gradient(90deg,#00FFEE,#00B8A0)',
                  boxShadow: isSoldOut ? '0 0 12px rgba(255,0,187,0.5)' : '0 0 12px rgba(0,255,238,0.4)',
                }}
              />
            </div>
          )}
        ...
        <div className="flex justify-between items-center">
          <span className="text-chrome-dim text-xs font-display font-bold tracking-widest uppercase">Price</span>
          <span className="font-display font-bold text-lg text-chrome">{MINT_PRICE_DISPLAY}</span>
        </div>

        {/* Wallet row */}
        {isConnected && solana.publicKey ? (
          <div className="flex items-center justify-between">
            <span className="text-chrome-dim/60 text-xs font-mono">{short(solana.publicKey)}</span>
            <button
              onClick={() => disconnect()}
          ...
          <button
            ...
            style={{
              background: 'linear-gradient(160deg,rgba(192,192,216,0.12),rgba(192,192,216,0.06))',
        ...
        {isConnected && !isSoldOut && status !== 'success' && (
          <button
            ...
            style={{
              ...
              boxShadow: mintDisabled ? 'none' : '0 0 32px rgba(0,255,238,0.3),0 1px 0 rgba(255,255,255,0.4) inset,0 -2px 0 rgba(0,0,0,0.2) inset',
        ...
        {status === 'success' && result && (
          <div
            ...
            style={{ background: 'rgba(0,255,238,0.05)', border: '1px solid rgba(0,255,238,0.2)' }}
          >
            <p className="font-display font-bold text-cyan-neon text-sm tracking-wide">Mint successful!</p>
            <p className="text-xs text-chrome-dim/70 font-mono">Asset: {short(result.assetAddress)}</p>
            <a
        ...
        {status === 'error' && error && (
          <div
            className="rounded-2xl p-4"
            style={{ background: 'rgba(255,0,187,0.05)', border: '1px solid rgba(255,0,187,0.2)' }}
          >
            <p className="font-display font-bold text-magenta-neon text-xs mb-1">Mint failed</p>
            <p className="text-chrome-dim/60 text-xs leading-relaxed break-words">{error}</p>
            <button
              onClick={reset}
