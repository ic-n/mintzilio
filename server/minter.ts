...
export class NftMinter {
  ...
  async syncSupply(): Promise<void> {
    ...
    const assets = await fetchAssetsByCollection(this.umi, this.collectionAddress);
    this.state.totalMinted = assets.length;
  }

  getStatus(): { totalMinted: number; maxSupply: number; isActive: boolean; price: string } {
    return {
  ...
  async buildMintTransaction(payerAddress: string): Promise<MintResult> {
    ...
    const collection = await fetchCollection(this.umi, this.collectionAddress);
   ...
   */
  async verifyMint(signature: Uint8Array): Promise<{ confirmed: boolean; signature: string }> {
    const result = await this.umi.rpc.confirmTransaction(signature, {
      strategy: { type: 'blockhash', ...(await this.umi.rpc.getLatestBlockhash()) },
    });
