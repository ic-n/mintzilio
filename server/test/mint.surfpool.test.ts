...
async function main() {
    ...
    assert(
        asset.name === 'Ponzilio #1',
        `NFT name should be "Ponzilio #1", got "${asset.name}"`
    );
    ...
    assert(
        status.totalMinted === 1,
        `totalMinted should be 1, got ${status.totalMinted}`
    );
    ...
    const result2 = await minter.buildMintTransaction(
    ...
    assert(
        ixCount >= 2,
        `Transaction should have at least 2 instructions (SPL transfer + create), got ${ixCount}`
    );
    log(
        `  Atomic tx instruction count: ${ixCount} (SPL transfer + Core create)`
    );
    ...
    try {
        ...
        assert(
            (e as Error).message === 'Minting is paused',
            'Should throw "Minting is paused"'
        );
